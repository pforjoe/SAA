...
class planParams:
    ...
    def __init__(self, policy_wgts, ret, vol, corr, symbols, funded_status,
                 ret_df=None, ):
        """
        ...
        self.eff_frontier_assetvols = None
        self.ports_df = None
        self.bnds_dict = self.set_bnds_dict()

    def get_pp_dict(self):
        vol_df = dm.pd.DataFrame(self.vol, index=self.symbols,
                                 columns=['Volatility'])
        ret_vol_df = dm.merge_dfs(dm.pd.DataFrame(self.ret), vol_df)

        return {'Policy Weights': dm.pd.DataFrame(self.policy_wgts,
                                                  index=self.symbols,
                                                  columns=['Weights']),
                'Asset/Liability Returns/Vol': util.add_sharpe_col(ret_vol_df),
                'Corr': dm.pd.DataFrame(self.corr, index=self.symbols,
                                        columns=self.symbols),
                'Cov': dm.pd.DataFrame(self.cov, index=self.symbols,
                                       columns=self.symbols),
                'Historical Returns': self.ret_df
                }

    def set_bnds_dict(self):
        return {'asset_list': [asset for asset in self.symbols[1:] if
                               asset != 'Cash'],
                'lower_bnd': ['{:.0%}'.format(x / 100) for x in
                              list(range(0, 105, 5))],
                'upper_bnd': ['{:.0%}'.format(x / 100) for x in
                              list(range(100, -5, -5))]
                }
    ...
    def portfolio_stats(self, weights):
        ...
        return np.array(
            [port_rets, port_vols, port_rets / port_vols, port_vars]).flatten()
    ...
    def optimize(self, fun, bnds, cons, method='SLSQP'):
        ...
        """
        if isinstance(bnds, tuple) is False:
            bnds = dm.transform_bnds(bnds)
        # bnds = dm.transform_bnds(bnds) if type(bnds) != tuple else bnds
        return sco.minimize(fun, self.policy_wgts.flatten(), method=method,
    ...
    def compute_eff_frontier(self, bnds, cons, num_ports=100):
        if isinstance(bnds, tuple) is False:
            bnds = dm.transform_bnds(bnds)
        # bnds = dm.transform_bnds(bnds) if type(bnds) != tuple else bnds
        # initial minimimum and maximum returns to define the bounds of the
        # efficient frontier
        ...
        for tr in self.eff_frontier_trets:
            ...
            ef_cons = ef_cons + (
                {'type': 'eq',
                 'fun': lambda x: self.portfolio_stats(x)[0] - tr},)
            ...
            asset_vols.append(
                self.portfolio_stats(self.get_asset_vol(opt_ef['x']))[1])
        ...
        self.ports_df = dm.get_ports_df(self.eff_frontier_trets,
                                        self.eff_frontier_tvols,
                                        self.eff_frontier_assetvols,
                                        self.eff_frontier_tweights,
                                        self.symbols)
