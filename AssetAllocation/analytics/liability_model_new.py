...
class liabilityModelNew():
    ...
    def __init__(self, plan, pbo_cashflows, disc_factors, sc_cashflows, contrb_pct,
        ...
        self.plan_pbo_cfs = ldi.offset(plan_pbo_cfs)
        self.plan_sc_cfs = ldi.offset(plan_sc_cfs)
        self.plan_cfs = self.plan_pbo_cfs + self.plan_sc_cfs*self.contrb_pct
        
        self.disc_factors = np.array(disc_factors)
        self.ldi_disc_factors = dm.get_cfs_time_col(self.plan_cfs)
        
        self.liab_curve = liab_curve
        self.ldi_liab_curve = self.get_ldi_liab_curve()

        self.asset_mv = asset_mv
        self.asset_returns = asset_returns
        
        self.ldi_pv = ldi.compute_pvs(self.plan_cfs,self.ldi_liab_curve, self.ldi_disc_factors)
        self.present_values = self.concat_data(self.compute_pvs(), self.ldi_pv)
        
        self.ldi_irr = ldi.compute_irr(self.ldi_pv, self.plan_cfs, self.ldi_disc_factors)
        self.irr_df = self.concat_data(self.compute_irr(), self.ldi_irr)
        
        # self.liab_mv = self.get_plan_liab_mv()
        self.liab_mv = self.get_ldi_liab_mv()
        
        self.ldi_liab_ret_df = ldi.compute_liab_ret(self.ldi_pv, self.plan_cfs)
        self.returns_ts = self.concat_data(self.compute_liab_ret(), self.get_ldi_return_ts())
    ...
    def get_ldi_liab_curve(self):
        temp_curve = self.liab_curve.append(self.liab_curve.iloc[[-1]*(len(self.plan_cfs)-len(self.liab_curve))] )
        temp_curve.reset_index(inplace=True)
        return temp_curve[list(self.liab_curve.columns)]

    def get_ldi_return_ts(self):
        ret_ts = self.ldi_liab_ret_df[['MTD']]
        ret_ts.columns  = ['Liability']
        return ret_ts

    def get_ldi_liab_mv(self):
        liab_mv = self.ldi_pv.copy()
    ...
    def compute_liab_ret(self):
        ...
        for i in range (0,len(self.present_values)-1):
            #compute liab pv return and add irr
