...
class liabilityModelNew():
    ...
    def __init__(self, sc_accrual, pbo_cashflows, disc_factors, sc_cashflows,
                 pbo_cfs_dict, sc_cfs_dict, contrb_pct, asset_mv, liab_mv_cfs,
                 asset_returns, liab_curve=pd.DataFrame,
        ...
        self.irr_df = self.concat_data(self.compute_irr(),
                                       self.new_pv_irr['IRR'])

        self.returns_ts = self.concat_data(self.compute_liab_ret(),
                                           self.compute_liab_ret_new(
                                               freq='1M'))
        self.qtd_liab_returns = self.compute_liab_ret_new(
            freq='1Q').transpose()
        self.ytd_liab_returns = self.compute_liab_ret_new(
            freq='1Y').transpose()

        self.liab_mv_cfs = pd.DataFrame(liab_mv_cfs)
        self.liab_mv = self.get_plan_liab_mv()
        self.funded_status = self.concat_fs_data(self.compute_funded_status(),
                                                 self.compute_funded_status_new())
    ...
    def get_liab_data_dict(self, pbo_cashflows, sc_cashflows):
        ...
        cf_frame = {'Total Cashflows': (
                    self.contrb_pct * sc_cashflows + pbo_cashflows),
                    'PBO Cashflows': pbo_cashflows,
                    'SC Cashflows': sc_cashflows}
        # cash flow dataframe
        ...
        return {'Present Values': self.present_values,
                'Liability Returns': self.returns_ts,
                'Liability YTD Returns': pd.DataFrame(
                    self.ytd_liab_returns).transpose(),
                'Liability QTD Returns': pd.DataFrame(
                    self.qtd_liab_returns).transpose(),

                'Liability Market Values': self.liab_mv, 'IRR': self.irr_df,
                'Asset Returns': self.asset_returns,
                'Asset YTD Returns': pd.DataFrame(
                    self.ytd_asset_returns).transpose(),
                'Asset QTD Returns': pd.DataFrame(
                    self.qtd_asset_returns).transpose(),
                'Asset Market Values': self.asset_mv,
                'Funded Status': self.funded_status,
        ...
        if self.disc_rates.empty:
            ...
            for col in self.liab_curve.columns:
                ...
                for j in range(0, len(self.total_cashflows)):
                    temp_pv += (self.total_cashflows[j] / (
                                (1 + self.liab_curve[col][j] / 100) **
                                self.disc_factors[j]))
                pv_dict[col] = temp_pv
            ...
            for i in range(len(self.disc_rates)):
                for j in range(0, len(self.total_cashflows)):
                    disc_rates_pv_array[i] += (self.total_cashflows[j] / (
                                (1 + self.disc_rates['IRR'][i]) **
                                self.disc_factors[j]))

            return pd.DataFrame(disc_rates_pv_array, columns=['Present Value'],
                                index=self.disc_rates.index)
    ...
    def compute_liab_ret(self):
        ...
        for i in range(0, len(self.present_values) - 1):
            ...
            liab_ret[i + 1] += self.irr_df['IRR'][i] / 12 + (
                        (self.present_values['Present Value'][i + 1]) /
                        self.present_values['Present Value'][i]) - 1
        # else:
        #     for i in range (0,len(self.present_values)-1):
        #         liab_ret[i+1] += self.disc_rates['IRR'][i]/12 + ((self.present_values['Present Value'][i+1])/self.present_values['Present Value'][i])-1

        return pd.DataFrame(liab_ret, columns=['Liability'],
                            index=self.present_values.index)
    ...
    def compute_disc_rates_pvs(self):
        ...
        for i in range(len(self.disc_rates)):
            for j in range(0, len(self.total_cashflows)):
                disc_rates_pv_list[i] += (self.total_cashflows[j] / (
                            (1 + self.disc_rates['IRR'][i]) **
                            self.disc_factors[j]))

        return pd.DataFrame(disc_rates_pv_list, columns=['Present Value'],
                            index=self.disc_rates.index)
    ...
    def irr(self, cfs, yrs, x0, **kwargs):
        ...
        return np.ndarray.item(
            fsolve(self.npv, x0=x0, args=(cfs, yrs), **kwargs))
        ...
        for j in range(len(self.disc_factors)):
            if j == 0:
                for i in range(j, len(self.total_cashflows)):
                    erf_pvs_list[j] += (self.total_cashflows[i] / (
                                (1 + self.irr_df['IRR'][-1]) **
                                self.disc_factors[i - j]))
                    asset_mv_list[j] = self.asset_mv.iloc[-1:]['Market Value'][
                        0]
                ...
                for i in range(j, len(self.total_cashflows)):
                    erf_pvs_list[j] += (self.total_cashflows[i] / (
                                (1 + self.irr_df['IRR'][-1]) **
                                self.disc_factors[i - j]))
                    asset_mv_list[j] = (asset_mv_list[j - 1] * (
                                1 + fulfill_return) ** self.disc_factors[
                                            0].tolist()) - \
                                       self.total_cashflows[j - 1]
    ...
    def compute_funded_status(self):
        ...
        fs_df['1Y FSV'] = gap_diff_percent.rolling(window=12).apply(
            get_ann_vol)
        fs_df['6mo FSV'] = gap_diff_percent.rolling(window=6).apply(
            get_ann_vol)
    ...
    def compute_funded_status_new(self):
        ...
        # compute funded status gap: liability(i.e PBO) - asset
        fs_df['Funded Status Gap'] = df_temp['Present Value'] - df_temp[
            'Market Value']
        fs_df.dropna(inplace=True)
    ...
    def get_plan_liab_mv(self):
        ...
        for i in list(range(0, len(self.liab_mv_cfs.columns))):
            ...
            pbo.append(
                self.npv(self.irr_df['IRR'][self.liab_mv_cfs.columns[i]] / 12,
                         cfs, yrs))
        return pd.DataFrame(pbo, index=self.liab_mv_cfs.columns,
                            columns=self.asset_mv.columns)
    ...
    def get_cf_table(self, cf_df, no_of_cols, accrual=False):
        ...
        cf_table = pd.DataFrame(columns=list(range(0, no_of_cols + 1)),
                                index=cf_df.index)
    ...
    def get_pv_irr(self):
        ...
        for year in dm.SHEET_LIST_LDI:
            ...
            pbo_series = pbo_series.append(
                self.pbo_cfs_dict[year].iloc[:no_of_cols + 1])
            ...
            pbo_table = self.get_cf_table(self.pbo_cfs_dict[year], no_of_cols)
            sc_table = self.get_cf_table(self.sc_cfs_dict[year], no_of_cols,
                                         accrual=self.sc_accrual)
            ...
            for col in total_cf_table.columns:
                ...
                temp_pv = total_cf_table[col].values / ((1 + self.liab_curve[
                    col].values / 100) ** self.disc_factors)
                pv[col] = [temp_pv.sum()]
        ...
        return {'Present Value': pv_df, 'IRR': irr_df,
                'PBO Series': pbo_series}
    ...
    def compute_liab_ret_new(self, freq='1M'):
        ...
        for row in list(range(0, len(self.pv_new))):
            ...
            if row - roll >= 0:
                liability_returns += [
                    (pv_series[row] + pbo) / pv_series[row - roll] - 1]
            else:
                continue

        return_df = pd.DataFrame(liability_returns, index=self.pv_new.index[
                                                          -len(
                                                              liability_returns):],
                                 columns=['Liability'])
    ...
    def compute_asset_ret(self, freq='1Q'):
        ...
        for row in list(range(0, len(price_series))):
            ...
            roll = lookback_window[row]
            asset_returns += [
                (price_series[row]) / price_series[row - roll] - 1]

        return_df = pd.DataFrame(asset_returns, index=price_df.index,
                                 columns=['Asset'])
