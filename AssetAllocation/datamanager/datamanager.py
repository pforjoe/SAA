...
UPPER_BND_LIST = ['15+ STRIPS', 'Long Corporate', 'Equity',
                  'Liquid Alternatives']
...
def get_mv_inputs_data(filename='inputs_test.xlsx', plan='IBT'):
    ...
    illiquidity = get_illiquidity_penalty(filepath,
                                          sheet_name=plan + " illiquidity")
    ...
    # Get weights
    weights = format_weights_index(get_weights(plan=plan),
                                   list(mkt_factor_prem.keys()))
...
def merge_dfs(main_df, new_df, dropna=True):
    merged_df = pd.merge(main_df, new_df, left_index=True, right_index=True,
                         how='outer')
    if dropna:
...
def get_asset_returns(filename='asset_return_data.xlsx',
                      sheet_name='Monthly Historical Returns'):
    """
    ...
    asset_ret_df = asset_ret_df[['15+ STRIPS', 'Long Corps', 'WN1 COMB Comdty',
                                 'Total EQ w/o Derivatives',
                                 'Total Liquid Alts',
                                 'Total Private Equity', 'Total Credit',
                                 'Total Real Estate', 'Cash', 'Equity Hedges']]
    asset_ret_df.columns = ['15+ STRIPS', 'Long Corporate',
                            'Ultra 30Y Futures', 'Equity',
                            'Liquid Alternatives',
                            'Private Equity', 'Credit', 'Real Estate', 'Cash',
                            'Hedges']
...
def get_liab_returns(filename='liability_return_data.xlsx', plan='IBT'):
    filepath = TS_FP + filename
    liab_ret_df = pd.read_excel(filepath, sheet_name=plan, usecols=[0, 1],
                                index_col=0)
...
def get_ports_df(rets, vols, asset_vols, weights, symbols, raw=True):
    if raw:
        return pd.DataFrame(
            np.column_stack([rets, vols, rets / vols, asset_vols, weights]),
            columns=['Return', 'Volatility', 'Sharpe',
                     'Asset Vol'] + symbols).rename_axis('Portfolio')
    else:
        return pd.DataFrame(np.column_stack(
            [100 * np.around(rets, 6), 100 * np.around(vols, 6),
             np.around(rets / vols, 6), 100 * np.around(weights, 6)]),
                            columns=['Return', 'Volatility', 'Sharpe',
                                     'Asset Vol'] + symbols).rename_axis(
            'Portfolio')


def format_ports_df(ports_df, ret_df):
    # rename Return & Volatility column to Excess Return & Surplus Volatility
    ports_df.columns = [col.replace('Return', 'Excess Return') for col in
                        ports_df.columns]
    ports_df.columns = [col.replace('Volatility', 'Surplus Volatility') for col
                        in ports_df.columns]
...
def get_prices_df(df_returns):
    ...
    for i in range(1, len(df_returns)):
        for col in df_returns.columns:
            df_prices[col][i] = (df_returns[col][i] + 1) * df_prices[col][
                i - 1]
    return df_prices
...
def compute_fs(plan='IBT'):
    liab_pv = pd.read_excel(TS_FP + 'liability_return_data.xlsx',
                            sheet_name=plan, index_col=0, usecols=[0, 2])
    asset_mv = pd.read_excel(TS_FP + 'plan_mkt_value_data.xlsx',
                             sheet_name=plan, index_col=0)
    fs_df = merge_dfs(asset_mv, liab_pv)
    return fs_df.iloc[-1:]['Market Value'][0] / \
        fs_df.iloc[-1:]['Present Value'][0]
...
def get_plan_asset_data(filename='plan_data.xlsx'):
    plan_mv_df = pd.read_excel(TS_FP + filename, sheet_name='mkt_value',
                               index_col=0)
    plan_ret_df = pd.read_excel(TS_FP + filename, sheet_name='return',
                                index_col=0)
...
def get_cf_data(cf_type='PBO', annual_cf_filename='annual_cashflows_data.xlsx',
    ...
    temp_cfs = pd.read_excel(TS_FP + monthly_cf_filename, sheet_name=cf_type,
                             index_col=0)
...
def monthize_cf_data(cf_type='PBO', filename='annual_cashflows_data.xlsx'):
    df_cfs = pd.read_excel(TS_FP + filename, sheet_name=cf_type,
                           index_col=0) / 12
...
def get_ftse_data(include_old=False, filename='ftse_data.xlsx'):
    df_ftse = pd.read_excel(TS_FP + filename, sheet_name='new_data',
                            index_col=0)

    if include_old:
        df_old_ftse = pd.read_excel(TS_FP + filename, sheet_name='old_data',
                                    index_col=0)
...
def generate_liab_curve(df_ftse, cfs):
    ...
    for col in df_ftse.columns:
        y = []
        interp = sp.interpolate.interp1d(dates, df_ftse[col],
                                         bounds_error=False,
                                         fill_value=np.nan)
...
def get_liab_model_data(plan='IBT', contrb_pct=.05):
    ...
    return {'pbo_cashflows': df_pbo_cfs[plan],
            'disc_factors': df_pbo_cfs['Time'],
            'sc_cashflows': df_sc_cfs[plan],
            'liab_curve': liab_curve, 'contrb_pct': contrb_pct,
            'asset_mv': asset_mv,
            'liab_mv_cfs': offset(plan_mv_cfs_dict[plan]),
            'asset_ret': asset_ret}
...
def get_n_year_df(liab_plan_data_dict, data='returns', n=3):
    if data == 'fs_data':
        ...
        for i in list(df_data_dict.keys())[0:-1]:
            n_year_df = merge_dfs(n_year_df,
                                  liab_plan_data_dict[df_data_dict[i]])
...
def switch_liab_dict(arg):
    switcher = {
        "returns": {'df1': 'Asset Returns', 'df2': 'Liability Returns',
                    'col_names': ['Asset', 'Liability']},
        "mv_pv_irr": {'df1': 'Asset Market Values', 'df2': 'Present Values',
                      'df3': 'IRR',
                      'col_names': ['Asset MV', 'Present Values', 'IRR']},
        "pv_irr": {'df1': 'Present Values', 'df2': 'IRR',
                   'col_names': ['Present Values', 'IRR']},
        "ytd_returns": {'df1': 'Asset YTD Returns',
                        'df2': 'Liability YTD Returns',
                        'col_names': ['Asset', 'Liability']},
        "qtd_returns": {'df1': 'Asset QTD Returns',
                        'df2': 'Liability QTD Returns',
                        'col_names': ['Asset', 'Liability']},
    }
...
def update_plan_data(report_name='Plan level Historical Returns.xls',
                     sheet_name='Plan level Historical Returns'):
    '''
    ...
    # rename columns
    plan_data.columns = ["Account Name", "Account Id", "Return Type", "Date",
                         "Market Value", "Monthly Return"]

    # rename each plan
    plan_data["Account Name"].replace(
        {"Total Retirement": "Retirement", "Total Pension": "Pension",
         "Total UPS/IBT FT Emp Pension": "IBT",
         "LDI ONLY-TotUSPenMinus401H": "Total",
         "UPS GT Total Consolidation": "Total Consolidation"}, inplace=True)

    # pivot table so that plans are in the columns and the rows are the market value/returns for each date
    mv_df = plan_data.pivot_table(values='Market Value', index='Date',
                                  columns='Account Name')

    # add back misc recievables
    add_misc_receiv(mv_df)

    ret_df = plan_data.pivot_table(values='Monthly Return', index='Date',
                                   columns='Account Name')
...
def add_misc_receiv(mv_df, filename='misc_receivables_data.xlsx'):
    misc_receiv_df = pd.read_excel(TS_FP + filename, sheet_name='misc_receiv',
                                   index_col=0)
...
def get_new_ftse_data(file_name='ftse-pension-discount-curve.xlsx'):
    ...
    new_ftse = pd.read_excel(UPDATE_FP + file_name,
                             sheet_name='Data - Current', skiprows=25,
                             header=1)
...
def update_ftse_data(file_name="ftse_data.xlsx"):
    ...
    prev_ftse = pd.read_excel(TS_FP + file_name,
                              sheet_name=['new_data', 'old_data'], index_col=0)
...
def merge_liab_data_df(liab_data_dict, plan_list):
    ...
    for plan in plan_list[1:]:
        for key in liab_data_dict[plan]:
            report_dict[key] = merge_dfs(report_dict[key],
                                         liab_data_dict[plan][key])
...
def transform_pbo_df(pbo_df):
    temp_df_list = [pbo_df.iloc[0:12 * len(SHEET_LIST)],
                    pbo_df.iloc[12 * len(SHEET_LIST):, ]]
...
def get_plan_pbo_dict(filename='past_pbo_cashflow_data.xlsx'):
    ...
    for plan in PLAN_LIST:
        ...
        for key in past_pbo_dict:
            temp_pbo_df = merge_dfs(temp_pbo_df, past_pbo_dict[key][plan],
                                    dropna=False)
...
def generate_liab_mv_dict(past_pbo_filename='past_pbo_cashflow_data.xlsx',
    ...
    for key in plan_pbo_dict:
        ...
        for year in plan_pbo_dict[key].columns:
            ...
            liab_cfs = pd.DataFrame(columns=list(range(1, no_of_cols + 1)),
                                    index=temp_cfs_df.index)

            for col in liab_cfs.columns:
                if key == 'IBT' and year == '2021' and col == 3:
                    liab_cfs[col] = transform_pbo_df(
                        plan_pbo_dict[key]['2021_1'])
                else:
                    liab_cfs[col] = temp_cfs_df['PBO'] + temp_cfs_df[
                        'SC'] * col
...
def get_plan_mv_cfs_dict(past_pbo_filename='past_pbo_cashflow_data.xlsx',
                         past_sc_filename='past_sc_cashflow_data.xlsx',
                         ftse_filename='ftse_data.xlsx'):
    liab_mv_dict = generate_liab_mv_dict(past_pbo_filename, past_sc_filename,
                                         ftse_filename)
    plan_mv_cfs_dict = {}
    for plan in liab_mv_dict:
        ...
        for year in liab_mv_dict[plan]:
            mv_cfs_df = merge_dfs(mv_cfs_df, liab_mv_dict[plan][year],
                                  dropna=False)
...
def get_plan_mv_file(plan_list=['Retirement', 'IBT', 'Pension', 'Total'],
                     file_name="liab_mv_cfs.xlsx"):
    plan_mv_dict = {}
    for plan in plan_list:
        plan_mv_dict[plan] = pd.read_excel(TS_FP + file_name, sheet_name=plan,
                                           index_col=0)
...
def aggregate_mv_cfs(plan_mv_cfs_dict):
    agg_df = pd.DataFrame(index=plan_mv_cfs_dict[PLAN_LIST[0]].index,
                          columns=plan_mv_cfs_dict[PLAN_LIST[0]].columns)
...
def transform_asset_returns(file_name='Historical Asset Class Returns.xls',
    ...
    # rename columns
    hist_ret.columns = ["Account Name", "Account Id", "Return Type", "Date",
                        "Market Value", "Monthly Return"]

    # pivot table so that plans are in the columns and the rows are the market value/returns for each date
    hist_ret_df = hist_ret.pivot_table(values='Monthly Return', index='Date',
                                       columns='Account Name')
...
def transform_index_data(file_name='index_data.xlsx', sheet_name='data'):
    index_data = pd.read_excel(DATA_FP + file_name, sheet_name=sheet_name,
                               index_col=0)
...
def transform_eq_hedges():
    ...
    eq_hedge_df = pd.read_excel(TS_FP + 'equity_hedge_data.xlsx',
                                sheet_name='Monthly Historical Returns',
                                usecols=['Date', 'Weighted Hedges'],
                                index_col=0)
    # rename columns
...
def update_asset_ret_data(file_name='asset_return_data.xlsx',
                          sheet_name='Monthly Historical Returns'):
    asset_ret_data = pd.read_excel(TS_FP + file_name, sheet_name=sheet_name,
                                   index_col=0)
...
def get_cf_dict_by_plan(filename='past_pbo_cashflow_data_for_ldi.xlsx'):
    ...
    for year in SHEET_LIST_LDI:
        temp_data_dict[year] = monthize_cf_data(cf_type=year,
                                                filename=filename)
...
def get_ldi_data(contrb_pct=1):
    ...
    pbo_data_dict = get_cf_dict_by_plan(
        filename='pbo_cashflow_data_for_ldi.xlsx')
    sc_data_dict = get_cf_dict_by_plan(
        filename='sc_cashflow_data_for_ldi.xlsx')

    disc_factors = get_disc_factors(pbo_data_dict['IBT']['2021'])

    df_ftse = get_ftse_data()
    liab_curve = generate_liab_curve(df_ftse,
                                     pbo_data_dict['IBT'][SHEET_LIST_LDI[-1]])
    plan_mv_cfs_dict = get_plan_mv_cfs_dict()

    return {'pbo_cfs_dict': pbo_data_dict, 'sc_cfs_dict': sc_data_dict,
            'disc_factors': disc_factors,
            'liab_curve': liab_curve, 'asset_mv': plan_asset_data['mkt_value'],
            'asset_ret': plan_asset_data['return'],
...
def get_future_sc(sc, n_years, contrib_pct=[], growth_factor=[]):
    ...
    if n_years > 0:
        ...
        for i in list(range(1, n_years + 1)):
            ...
            # multiply previous years service cost by growth factor
            temp_df[year] = sc_df.iloc[:, i - 1].shift(12) * (
                        1 + growth_factor[i])
            # subtract contributions
            temp_df[year] = temp_df[year] * (1 - contrib_pct[i])
            temp_df.fillna(0, inplace=True)

            sc_df = sc_df.merge(temp_df, how="outer", left_index=True,
                                right_index=True)
