...
# no_illiquidity True --> no illiquidity penalty applied to asset return
no_illiquidity = False

# no_mrp True --> no market risk premium applied to asset return
no_mrp = False
...
for PLAN in ['IBT','Retirement','Pension']:

    if future_sc:
        n_years = 0
        if PLAN == "IBT":
            contrib_pct = [0.24635519045240]
        if PLAN == "Retirement":
            contrib_pct = [16.1692378538607]
        if PLAN == "Pension":
            contrib_pct = [0.700714860622032]
    ...
    # COMPUTE PLAN INPUTS                                                         #
    ###############################################################################
    pp_inputs = summary.get_pp_inputs(liab_model,PLAN, priv_mrp0 =
    priv_mrp0, no_illiquidity=no_illiquidity, no_mrp = no_mrp)
   ...
   ###############################################################################
    ...
    filename = ' stochmv_ef_report_constrained_priv_ONLY_fi_fixed'
    if unconstrained:
        filename = ' stochmv_ef_report_unconstrained_FI_fixed'
