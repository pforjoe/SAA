# -*- coding: utf-8 -*-
"""
Created on Wed Sep 29 22:22:20 2021
...
#Get FI inputs
fi_df = pd.read_excel('inputs.xlsx', sheet_name='fi', index_col=0)
fi_df['Current Vol'] = fi_df['Historical Vol'] / fi_df['Historical Spread'] * fi_df['Current Spread']
fi_df = fi_df.fillna(0)

#Get RSA inputs
rsa_df = pd.read_excel('inputs.xlsx', sheet_name='rsa', index_col=0)

#Get Rates Vol inputs
rv_df = pd.read_excel('inputs.xlsx', sheet_name='rates_vol', index_col=0)
rv_df['Pros Rate Vol'] = rv_df['3mo Tsy Futures Options Vol'] * rv_df['12mo expiry'] / rv_df['3mo expiry']

#Get vol definitions
vol_df = pd.read_excel('inputs.xlsx', sheet_name='vol_defs')
...
rsa_cash_list = list(rsa_df.index) + ['Cash']
for asset in rsa_cash_list:
...
corr = pd.read_excel('inputs.xlsx', sheet_name='corr', index_col=0).to_numpy()

#Compute Covariance
cov = compute_cov(vol_assump, corr)

#Compute plan Volatility
output_df = pd.read_excel('inputs.xlsx', sheet_name='weights', index_col=0)
output_df['FS AdjWeights'] = output_df['Weights'] * output_df['Factor Loadings']
asset_list = list(output_df.index)

vol_list=compute_plan_vol(vol_df, cov, asset_list)

output_df['Vol'] = vol_list
comp_weights = vol_df.iloc[:,3:].to_numpy()

#Compute plan correlations
corr_array = compute_plan_corr(vol_list, comp_weights)
...
for asset in rsa_cash_list:
    beta = compute_beta(asset, output_df, corr_df)
    print('{}: {}'.format(asset, beta))
    ret_list.append(compute_rsa_return(beta))

output_df['Return'] = compute_plan_return(fi_df,corr_df, output_df,rsa_cash_list)
